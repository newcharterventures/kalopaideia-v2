...
export function getUser(userId, product) {
  if (!userId) return null;
  const u = db().prepare("SELECT * FROM users WHERE id = ?").get(userId);
  if (!u) return null;
  // per-product subscription lookup — Mansion/Kalopaideia paywalls split 2026-08-06
  const sub = db().prepare("SELECT * FROM subscriptions WHERE user_id = ? AND product = ?").get(userId, product);
  return {
    ...u,
...
export function setSubscription({ userId, product, stripe_subscription_id, status, tier, current_period_end, cancel_at_period_end }) {
  db().prepare(`
    INSERT INTO subscriptions (user_id, product, stripe_subscription_id, status, tier, current_period_end, cancel_at_period_end, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
  ...
  `).run(userId, product, stripe_subscription_id, status, tier, current_period_end, cancel_at_period_end ? 1 : 0);
}
...
// ─────────────────────────────────────────────────────────────────
// Patron / sponsor contributions (Jae 2026-05-20).
// Reads the SAME patron_events table the Mansion's webhook writes to.
// Both sites share one Stripe webhook (it lives on the Mansion side)
// and the shared accounts.db lets us render the progress bar here.
// ─────────────────────────────────────────────────────────────────

export function getPatronSummary(site) {
  const d = db();
  if (!d) return { site, this_month: { patrons: 0, total_cents: 0 }, lifetime: { patrons: 0, total_cents: 0 } };
  const now = new Date();
  const monthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)).toISOString();
  // Table may not exist yet on a fresh DB — return zeros if so.
  try {
    const row = d.prepare(`
      SELECT COUNT(DISTINCT COALESCE(stripe_customer_id, stripe_event_id)) AS patrons,
             COALESCE(SUM(amount_cents), 0) AS total_cents
      FROM patron_events
      WHERE site = ? AND occurred_at >= ?
    `).get(site, monthStart);
    ...
    return {
      site,
      this_month: { patrons: row.patrons || 0, total_cents: row.total_cents || 0 },
      lifetime:   { patrons: lifetime.patrons || 0, total_cents: lifetime.total_cents || 0 },
    };
  } catch (e) {
    if (String(e.message).includes('no such table')) {
      return { site, this_month: { patrons: 0, total_cents: 0 }, lifetime: { patrons: 0, total_cents: 0 } };
    }
    throw e;
  }
}
