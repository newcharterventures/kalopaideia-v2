// lib/commerce.js — Kalopaideia commerce: auth, akousma, checkout, account, webhook.
//
// Mounts under /paideia/* (alongside the existing static site).
// Sold as 'Kalopaideia All-Access' — a single $12.99/mo subscription that
// unlocks every Library work AND the Curriculum (Jae 2026-05-20: $12.99/mo +
// $99.99/yr; was $15.99, was $11.99 before that).
// Stripe charges new subscribers $12.99; existing $15.99/$11.99 subscribers are
// grandfathered. Originally sold as 'The Akousma' — a single $11.99/mo that unlocks every
// classical work here and at The Reading Mansion. Per Jae 2026-05-12, the
// per-work $9.99 pricing was retired in favor of subscription-only.
// Shares accounts.db + Stoa catalog (backend kept as 'stoa' per Jae 2026-05-12
// rebrand decision: rename user-visible only, leave backend identifiers).
// Legacy /store → /akousma 301 redirects at the bottom of this file.
...
// ─── session store (kalopaideia-local sessions; shares users with mansion via accounts.db) ───
...
export function buildCommerce({ basePath = "/paideia" } = {}) {
  ...
  router.use(session({
    name: "kalopaideia.sid",
    store: new SQLiteStore({ client: sessionDb, expired: { clear: true, intervalMs: 15 * 60 * 1000 } }),
    secret: process.env.SESSION_SECRET || "kalopaideia-dev-secret-change-me",
    resave: false,
  ...
  router.use((req, res, next) => {
    req.user = req.session?.userId ? getUser(req.session.userId, 'kalopaideia') : null;
    next();
  });
  ...
  // Match app.js's toRoman + issueLines so the server-rendered C masthead
  // shows the same date/issue/vol/anno as the client-rendered homepage.
  ...
  function issueLines() {
    ...
    const dayLabel = dt.toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric", timeZone: "UTC" });
    return {
...
<html lang="en"><head>
  ...
  <link rel="stylesheet" href="${basePath}/styles.css?v=1779378800">
  <link rel="stylesheet" href="${basePath}/v2-design.css?v=1779378800">
  <link rel="stylesheet" href="${basePath}/akousma-v2.css?v=1779378800">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500&family=Cinzel:wght@400;500;600;700&family=Bodoni+Moda:ital,opsz,wght@0,6..96,400;0,6..96,500;0,6..96,600;0,6..96,700;0,6..96,800;1,6..96,400&family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;1,8..60,400&family=Inter:wght@400;500;600;700&display=swap">
  <style>
    ...
    /* --- OAuth sign-in buttons (2026-08-17): real brand marks + colors,
       not the beige commerce-btn treatment. Each provider keeps its own
       official button convention (Google: white+border; Microsoft: white
       +border; Apple: black). --- */
    .oauth-btn {
      display: flex; align-items: center; justify-content: center; gap: 12px;
      width: 100%; box-sizing: border-box; padding: 11px 16px; margin-bottom: 10px;
      border-radius: 4px; border: 1px solid #dadce0; background: #fff; color: #3c4043;
      font-family: "Inter", -apple-system, sans-serif; font-size: 14px; font-weight: 500;
      cursor: pointer; text-decoration: none; transition: box-shadow .15s, background .15s;
    }
    .oauth-btn:hover { box-shadow: 0 1px 3px rgba(0,0,0,.15); background: #f8f9fa; }
    .oauth-btn svg { flex: none; }
    .oauth-btn.apple { background: #000; color: #fff; border-color: #000; }
    .oauth-btn.apple:hover { background: #1a1a1a; }
    .oauth-btn.microsoft { background: #fff; color: #5e5e5e; border-color: #8c8c8c; }
    .oauth-btn.microsoft:hover { background: #f5f5f5; }
    .oauth-divider { display: flex; align-items: center; gap: 12px; margin: 20px 0; }
    .oauth-divider::before, .oauth-divider::after { content: ""; flex: 1; height: 1px; background: #d8cfbe; }
    .oauth-divider span { font-family: "Inter", sans-serif; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #8a8070; }
  </style>
</head><body>
${(() => {
  const lines = issueLines();
  return `<header class="masthead masthead-athenaeum">
  <img class="mast-watermark" src="${basePath}/img/masthead/temple-ruin.svg" alt="" aria-hidden="true" />
  <div class="mast-content">
    <div class="mast-grid">
      <div class="mast-center">
        <div class="mast-eyebrow">A daily book of words \u00b7 ten tongues</div>
        <h1 class="mast-name"><a href="${basePath}/" aria-label="Kalopaideia home">KALOPAIDEIA</a></h1>
        <span class="mast-greek"><em>\u03ba\u03b1\u03bb\u1f74 \u03c0\u03b1\u03b9\u03b4\u03b5\u03af\u03b1</em> \u2014 the cultivation of mind through classical letters</span>
      </div>
    </div>
    <div class="mast-ledger">
      <div class="ledger-left">
        <span class="mast-issue">${lines.issue}</span>
        <span class="mast-vol">${lines.vol}</span>
      </div>
      <div class="ledger-right">
        <span class="mast-day">${esc(lines.day)}</span>
        <span class="mast-anno">${lines.anno}</span>
      </div>
    </div>
    <nav class="nav mast-nav">
      <a href="${basePath}/greek">Greek</a>
      <a href="${basePath}/latin">Latin</a>
      <a href="${basePath}/french">French</a>
      <a href="${basePath}/german">German</a>
      <a href="${basePath}/italian">Italian</a>
      <a href="${basePath}/oldenglish">Olde English</a>
      <a href="${basePath}/middleenglish">Middle English</a>
      <a class="util" href="${basePath}/akousma">The Akousma</a>
      <a class="util" href="${basePath}/curriculum.html">Curriculum</a>
      <a class="util" href="${basePath}/commonplace.html">Commonplace</a>
      <a class="util" href="${basePath}/support.html">Support</a>
      <a class="util" href="${basePath}/account">${accountLabel}</a>
      <a class="util" href="${basePath}/about.html">About</a>
    </nav>
  </div>
</header>`;
})()}
...
<footer class="colophon">
  <div class="name">KALOPAIDEIA</div>
  <div class="tagline"><em>καλὴ παιδεία</em> <span class="sep">·</span> the cultivation of mind through classical letters</div>
  <div class="est">EST. MMXXIV <span class="sep">·</span> ATHENS &amp; OXFORD &amp; ELSEWHERE <span class="sep">·</span> <span class="imprint">NEW CHARTER VENTURES</span></div>
  <div class="legal">
    Anno Domini MMXXVI <span class="sep">·</span>
    <a href="${basePath}/about.html">About</a> <span class="sep">·</span>
    <a href="${basePath}/contact.html">Contact</a> <span class="sep">·</span>
    <a href="${basePath}/credits.html">Credits</a> <span class="sep">·</span>
    <a href="${basePath}/terms.html">Terms</a>
  </div>
</footer>
<script src="${basePath}/nav.js?v=1778651397"></script>
</body></html>`;
  }

  function priceLabel(cents) { return "$" + (cents / 100).toFixed(2); }

  // ─── /paideia/akousma — catalog index REMOVED (Jae 2026-08-18) ───
  // The standalone "browse every work" catalog page duplicated the per-
  // language Library tabs (which are the actual discovery surface now)
  // and confused readers with its own separate "subscribe" CTA. The route
  // now redirects straight to the homepage rather than 404, since it's
  // still linked from old bookmarks / emails / the store->akousma legacy
  // redirect below. Per-book detail pages (/akousma/:id) stay live —
  // they're still the checkout/CTA target from language-page book cards.
  router.get("/akousma", (req, res) => res.redirect(302, `${basePath}/`));

  // ─── /paideia/akousma/:id — single work detail (subscription-only) ───
  router.get("/akousma/:id", (req, res) => {
    const w = stoaById(req.params.id);
    if (!w) return res.status(404).send(shell("Not found", `<h1>Not found</h1><p><a href="${basePath}/akousma">← The Akousma</a></p>`, req.user));
    const user = req.user;
...
<div style="display: grid; grid-template-columns: 36px 1fr 1fr; gap: 18px; padding: 12px 0; border-bottom: 1px solid #e6dfd0; align-items: baseline;">
  ...
  <span style="font-family: 'Source Serif 4', serif; font-size: 16px; font-style: italic; color: #524a3e; line-height: 1.6;">${esc(ln.english)}</span>
    ...
    // Per Jae 2026-08-18: after becoming a member from THIS book's page,
    // land them straight in the reader for THIS book, not /account.
    const nextReader = encodeURIComponent(`${basePath}/read/${w.id}`);
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access?next=${nextReader}" style="display:inline;">
           <button class="commerce-btn" type="submit">Become a Member — $12.99 / month</button>
         </form>`
      : `<button class="commerce-btn" type="button" disabled style="opacity:0.55; cursor:not-allowed;">Become a Member — coming soon</button>`;
    const cta = has
...
${sampleHtml}
<p class="commerce-meta" style="margin-top: 16px;">${lineCount - 5} more lines open with membership — $12.99 a month or $99.99 a year, every work, present and future.</p>
`;
    res.send(shell(w.title, body, req.user));
  });
  ...
  router.get("/account", (req, res) => {
    ...
    const stripeReady = stripeConfigured();
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline;"><button class="commerce-btn" type="submit">Become a Member — $12.99 / month</button></form>`
      : `<button class="commerce-btn" type="button" disabled style="opacity:0.55; cursor:not-allowed;">Become a Member — coming soon</button>`;
    let subBlock;
    if (subStatus === "active") {
      ...
      subBlock = `<div class="commerce-card"><h3>Membership — $12.99 / month</h3><p style="font-style:italic; color:#6b5d4a;">Unlocks every classical work, here and at <a class="commerce-link" href="https://newcharterventures.com/mansion/wanderings/akousma">The Reading Mansion</a>. Cancel any time.</p>${subscribeBtn}</div>`;
    }
    ...
    const holdingsList = holdings.length
      ...
      : `<p class="commerce-empty">No books yet. <a class="commerce-link" href="${basePath}/akousma">Visit The Akousma.</a></p>`;
...
<p><b style="color:#1f1b16">The Akousma</b> is $12.99 per month or $99.99 per year (save $56). Every classical work, present and future, opens to you here and at The Reading Mansion. Cancel any time.</p>
<p style="font-family: 'Cormorant Garamond', serif; font-style: italic; color: #8a8070; margin-top: 18px;">Subscriptions don't carry refunds for past billing periods — cancel any time and access ends at the next renewal. If you were charged in error, contact us and we'll fix it.</p>
</div>
`;
    res.send(shell("Account", body, req.user));
  });
  ...
  router.get("/login", (req, res) => {
    ...
    const MS_SVG = `<svg width="18" height="18" viewBox="0 0 21 21"><rect x="1" y="1" width="9" height="9" fill="#f25022"/><rect x="11" y="1" width="9" height="9" fill="#7fba00"/><rect x="1" y="11" width="9" height="9" fill="#00a4ef"/><rect x="11" y="11" width="9" height="9" fill="#ffb900"/></svg>`;
    const APPLE_SVG = `<svg width="17" height="17" viewBox="0 0 17 17"><path fill="currentColor" d="M14.94 12.02c-.26.6-.4.87-.75 1.4-.48.72-1.16 1.62-2 1.63-.75.01-.95-.49-1.97-.48-1.02.01-1.24.5-1.99.48-.84-.02-1.48-.83-1.96-1.55-1.35-2-1.49-4.35-.66-5.6.59-.9 1.52-1.42 2.39-1.42.88 0 1.44.48 2.16.48.7 0 1.14-.48 2.16-.48.78 0 1.6.42 2.19 1.15-1.93 1.06-1.62 3.81.43 4.39zM10.99 4.6c.35-.45.63-1.08.53-1.72-.57.04-1.24.4-1.63.86-.36.42-.66 1.06-.54 1.68.63.05 1.28-.32 1.64-.82z"/></svg>`;
    ...
    if (!isAuthConfigured()) {
      ...
      inner = `${errMsg}
<a class="oauth-btn" href="${basePath}/auth/oauth/google${next}">${GOOGLE_SVG}Continue with Google</a>
<a class="oauth-btn microsoft" href="${basePath}/auth/oauth/azure${next}">${MS_SVG}Continue with Microsoft</a>
<a class="oauth-btn apple" href="${basePath}/auth/oauth/apple${next}">${APPLE_SVG}Continue with Apple</a>
...
<form method="POST" action="${basePath}/auth/magic-link${next}" class="commerce-form">
  ...
  <button type="submit" class="commerce-btn" style="width:100%;">Send a sign-in link</button>
</form>`;
    }

    const body = `
<div style="max-width: 400px; margin: 60px auto; text-align:center;">
  <h1>Sign in to Kalopaideia</h1>
  <p class="commerce-sub">Choose an account, or use a sign-in link.</p>
  ...
  <p style="margin-top: 20px; font-family: 'Cormorant Garamond', serif; font-style: italic; color: #8a8070; font-size: 14px;">One sign-in works at <a class="commerce-link" href="https://newcharterventures.com/mansion/">The Reading Mansion</a> too.</p>
    ...
    res.send(shell("Sign in", body, req.user));
  });
  ...
  router.get("/auth/oauth/:provider", async (req, res, next) => {
    ...
    if (!"google apple github azure".split(" ").includes(provider)) return res.status(400).send("unknown provider");
  ...
  router.post("/logout", (req, res) => {
    req.session.destroy(() => {
      res.clearCookie("kalopaideia.sid");
  ...
  function notConfiguredPage(res, summary) {
    return res.send(shell("Almost ready", `<h1>Almost ready</h1><p class="commerce-sub">Set STRIPE_SECRET_KEY to enable purchases.</p><p style="font-style: italic; color: #6b5d4a;">${esc(summary)}</p><a class="commerce-btn quiet" href="javascript:history.back()">← Back</a>`, null));
  }
  ...
  router.post("/checkout/all-access", async (req, res, next) => {
    try {
      if (!stripeConfigured()) return notConfiguredPage(res, "The Akousma — $12.99 / month");
      const user = requireUser(req, res); if (!user) return;
  ...
  router.post("/webhooks/stripe",
    ...
    async (req, res) => {
      ...
      catch (e) { console.error("[kalopaideia webhook]", e); res.status(500).send("handler error"); }
    }
  );

  // ─── /paideia/read/:id — owners-only line-by-line reader ───
  // (DISABLED: the existing static reader at /paideia/read/:id has been the
  // free public reader for over a year. Kalopaideia content stays free as
  // pedagogical content. Subscription pricing applies to the Mansion's
  // Stoa surface, not to Kalopaideia's reader. Routing through this commerce
  // handler was hijacking the URL and redirecting users to the store —
  // breaking the voice/pronunciation/notes UI Jae has been building for
  // a year. Reverting.)
  //
  // Original handler kept below as `_disabledReadHandler` for reference;
  // it's never registered on the router.
  function _disabledReadHandler(req, res) {
    const w = stoaById(req.params.id);
    if (!w) return res.status(404).send(shell("Not found", `<h1>Not found</h1>`, req.user));
    const user = req.user;
    const has = hasStoaAccess(user, w);
...
`;
  ...
  // ─── Legacy /store → /akousma redirects (301, per Jae 2026-05-12 rebrand) ───
  router.get("/store",      (req, res) => res.redirect(301, `${basePath}/akousma`));
  router.get("/store/",     (req, res) => res.redirect(301, `${basePath}/akousma`));
  router.get("/store/:id",  (req, res) => res.redirect(301, `${basePath}/akousma/${encodeURIComponent(req.params.id)}`));

  // ─── /paideia/akousma/ trailing-slash variant ───
  router.get("/akousma/",   (req, res) => res.redirect(`${basePath}/akousma`));
