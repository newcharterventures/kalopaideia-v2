...
// Sold as 'The Akousma' — a single $11.99/mo subscription that unlocks every
...
<script src="${basePath}/nav.js?v=1778561010"></script>
</body></html>`;
  }
  ...
  // per-work + subscription to subscription-only at $11.99/month.
...
<div class="commerce-row${w.is_gateway ? " gateway" : ""}">
  ...
  <span class="work-price">${w.is_gateway ? "Free" : "Subscriber"}</span>
    ...
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline; margin-top: 8px;"><button class="commerce-btn" type="submit">Subscribe — $11.99/month</button></form>`
    ...
    const akousmaCard = sub
      ...
      : `<div class="commerce-card"><h3>The Akousma — $11.99 / month</h3><p style="font-style:italic; color:#6b5d4a;">Every classical work, present and future. Cancel any time. The same subscription unlocks <a class="commerce-link" href="https://newcharterventures.com/mansion/wanderings/akousma">The Akousma at The Reading Mansion</a>.</p>${subscribeBtn}</div>`;
...
<div class="commerce-banner">
  <span><b>Odyssey, Book 1</b> is free for everyone — sign in and it's yours forever.<br>Every other work below opens with an Akousma subscription, $11.99 a month. One subscription, both sites.</span>
</div>
  ...
  <span style="font-family: 'Source Serif 4', serif; font-size: 16px; font-style: italic; color: #524a3e; line-height: 1.6;">${esc(ln.english)}</span>
    ...
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline;">
           <button class="commerce-btn" type="submit">Subscribe to The Akousma — $11.99/month</button>
         </form>`
...
${sampleHtml}
<p class="commerce-meta" style="margin-top: 16px;">${lineCount - 5} more lines open with an Akousma subscription — $11.99 a month, every work, present and future.</p>
`;
  ...
  router.get("/account", (req, res) => {
    ...
    const stripeReady = stripeConfigured();
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline;"><button class="commerce-btn" type="submit">Subscribe — $11.99/month</button></form>`
      : `<button class="commerce-btn" type="button" disabled style="opacity:0.55; cursor:not-allowed;">Subscribe — coming soon</button>`;
    let subBlock;
    if (subStatus === "active") {
      ...
      subBlock = `<div class="commerce-card"><h3>The Akousma — $11.99 / month</h3><p style="font-style:italic; color:#6b5d4a;">Unlocks every classical work, here and at <a class="commerce-link" href="https://newcharterventures.com/mansion/wanderings/akousma">The Reading Mansion</a>. Cancel any time.</p>${subscribeBtn}</div>`;
    }
...
<p><b style="color:#1f1b16">The Akousma</b> is $11.99 per month. Every classical work, present and future, opens to you here and at The Reading Mansion. Cancel any time.</p>
...
<div style="max-width: 420px; margin: 60px auto; text-align:center;">
  ...
  router.post("/checkout/all-access", async (req, res, next) => {
    try {
      if (!stripeConfigured()) return notConfiguredPage(res, "The Akousma — $11.99/month");
      const user = requireUser(req, res); if (!user) return;
