...
<script src="${basePath}/nav.js?v=1778651397"></script>
  ...
  // ─── /paideia/akousma — full classical-works catalog (subscription-only) ───
  // Per Jae 2026-05-12: the "Store" was rebranded to "The Akousma" across both
  // Kalopaideia and The Reading Mansion. The pricing model was simplified from
  // per-work + subscription to subscription-only at $12.99 / month.
  router.get("/akousma", (req, res) => {
...
<div class="commerce-row${w.is_gateway ? " gateway" : ""}">
  <span class="work-title"><a href="${basePath}/akousma/${esc(w.id)}">${esc(w.title)}</a></span>
  <span class="work-meta">${esc(w.author)} · ${esc(w.date_composed || "")} · ${w.line_count} lines</span>
  <span class="work-price">${w.is_gateway ? "Free" : "Subscriber"}</span>
</div>`).join("");
    ...
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline; margin-top: 8px;"><button class="commerce-btn" type="submit">Subscribe — $12.99 / month</button></form>`
      : `<button class="commerce-btn" type="button" disabled style="opacity:0.55; cursor:not-allowed; margin-top: 8px;">Subscribe — coming soon</button>`;
    const akousmaCard = sub
      ? `<div class="commerce-card"><h3>The Akousma · Active</h3><p style="font-style:italic; color:#6b5d4a;">Every work below is open to you. Renews monthly until you cancel.</p><a class="commerce-btn quiet" href="${basePath}/account">Manage subscription</a></div>`
      : `<div class="commerce-card"><h3>The Akousma — $12.99 / month or $99.99 / year</h3><p style="font-style:italic; color:#6b5d4a;">Every classical work, present and future. Cancel any time. The same subscription unlocks <a class="commerce-link" href="https://newcharterventures.com/mansion/wanderings/akousma">The Akousma at The Reading Mansion</a>. Or <a class="commerce-link" href="${basePath}/support.html">become a sponsor</a> to fund new texts and recordings.</p>${subscribeBtn}</div>`;
...
<div class="commerce-banner">
  <span><b>Odyssey, Book 1</b> is free for everyone — sign in and it's yours forever.<br>Every other work below opens with an Akousma subscription, $12.99 a month or $99.99 a year. One subscription, both sites.</span>
</div>

${sections}
`;
    res.send(shell("The Akousma", body, req.user));
  });
...
<div style="display: grid; grid-template-columns: 36px 1fr 1fr; gap: 18px; padding: 12px 0; border-bottom: 1px solid #e6dfd0; align-items: baseline;">
  ...
  <span style="font-family: 'Source Serif 4', serif; font-size: 16px; font-style: italic; color: #524a3e; line-height: 1.6;">${esc(ln.english)}</span>
    ...
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline;">
           <button class="commerce-btn" type="submit">Subscribe to The Akousma — $12.99 / month</button>
         </form>`
      : `<button class="commerce-btn" type="button" disabled style="opacity:0.55; cursor:not-allowed;">Subscribe — coming soon</button>`;
    const cta = has
      ? `<a class="commerce-btn" href="${basePath}/read/${esc(w.id)}">Open the reader</a>`
      : subscribeBtn;

    const body = `
<p class="commerce-nav"><a href="${basePath}/akousma">← The Akousma</a></p>
<h1>${esc(w.title)}</h1>
...
${sampleHtml}
<p class="commerce-meta" style="margin-top: 16px;">${lineCount - 5} more lines open with an Akousma subscription — $12.99 a month or $99.99 a year, every work, present and future.</p>
`;
  ...
  router.get("/account", (req, res) => {
    ...
    const stripeReady = stripeConfigured();
    const subscribeBtn = stripeReady
      ? `<form method="POST" action="${basePath}/checkout/all-access" style="display:inline;"><button class="commerce-btn" type="submit">Subscribe — $12.99 / month</button></form>`
      : `<button class="commerce-btn" type="button" disabled style="opacity:0.55; cursor:not-allowed;">Subscribe — coming soon</button>`;
    let subBlock;
    if (subStatus === "active") {
      ...
      subBlock = `<div class="commerce-card"><h3>The Akousma — $12.99 / month</h3><p style="font-style:italic; color:#6b5d4a;">Unlocks every classical work, here and at <a class="commerce-link" href="https://newcharterventures.com/mansion/wanderings/akousma">The Reading Mansion</a>. Cancel any time.</p>${subscribeBtn}</div>`;
    }
...
<div style="max-width: 420px; margin: 60px auto; text-align:center;">
  <h1>Sign in</h1>
  <p class="commerce-sub">A sign-in link, a Google sign-in, an Apple sign-in.</p>
