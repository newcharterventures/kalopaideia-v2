  ...
  <div class="mast-content">
    <div class="mast-grid">
      <div class="mast-center">
        <div class="mast-eyebrow">A daily book of words \u00b7 seven tongues</div>
    ...
    <nav class="nav mast-nav">
      ...
      <a class="util" href="${basePath}/store">Store</a>
...
<div class="commerce-banner">
  <span><b>Odyssey, Book 1</b> is free for everyone — sign in and it's yours forever.<br>Other works are <b>$9.99 each</b>, kept for life. Or All-Access at $11.99 a month.</span>
</div>

${sections}
`;
    res.send(shell("Store", body, req.user));
  });

  // ─── /paideia/store/:id — single work detail ───
  router.get("/store/:id", (req, res) => {
    const w = stoaById(req.params.id);
    if (!w) return res.status(404).send(shell("Not found", `<h1>Not found</h1><p><a href="${basePath}/store">← Store</a></p>`, req.user));
    const user = req.user;
    ...
    const cta = has
      ...
      : `<form method="POST" action="${basePath}/checkout/stoa-book/${esc(w.id)}" style="display:inline;">
         ...
         <form method="POST" action="${basePath}/checkout/all-access" style="display:inline; margin-left: 8px;">
           <button class="commerce-btn quiet" type="submit">All-Access — $11.99/mo</button>
         </form>`;
...
`;
  ...
  router.get("/account", (req, res) => {
    ...
    if (subStatus === "active") {
      ...
      subBlock = `<div class="commerce-card"><h3>All-Access — $11.99 / month</h3><p style="font-style:italic; color:#6b5d4a;">Unlocks every classical work, here and at <a class="commerce-link" href="https://newcharterventures.com/mansion/wanderings/stoa">The Reading Mansion</a>. Cancel any time.</p><form method="POST" action="${basePath}/checkout/all-access" style="display:inline;"><button class="commerce-btn" type="submit">Subscribe</button></form></div>`;
    }
...
<p><b style="color:#1f1b16">All-Access</b> is $11.99 per month. Cancel any time. The works you bought separately stay yours.</p>
...
<div style="max-width: 420px; margin: 60px auto; text-align:center;">
  ...
  router.post("/checkout/all-access", async (req, res, next) => {
    try {
      if (!stripeConfigured()) return notConfiguredPage(res, "Kalopaideia All-Access — $11.99/month");
      const user = requireUser(req, res); if (!user) return;
