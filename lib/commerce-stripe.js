...
export async function createAllAccessCheckout({ user, req }) {
  if (!stripe) throw new Error("Stripe not configured");
  const customerId = await findOrCreateCustomer(user);
  // Per Jae 2026-08-18: when a book-specific CTA sends the reader through
  // checkout, ?next=/paideia/read/<id> carries the intended destination so
  // success lands them straight in the reader for the book they clicked,
  // not the generic /account page. Only accept same-origin /paideia/ paths
  // to avoid an open-redirect via a crafted ?next= value.
  const rawNext = typeof req.query?.next === "string" ? req.query.next : "";
  const nextPath = rawNext.startsWith("/paideia/") ? rawNext : "";
  const successUrl = nextPath
    ? `${originOf(req)}${nextPath}${nextPath.includes("?") ? "&" : "?"}subscribed=1`
    : `${originOf(req)}/paideia/account?subscribed=1`;
  return stripe.checkout.sessions.create({
    mode: "subscription",
    customer: customerId,
    line_items: [{
