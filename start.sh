...
# Kalopaideia's own session secret (kalopaideia.sid cookie). Different from Mansion's.
# This is fresh each session unless overridden in the systemd unit.
export SESSION_SECRET="${SESSION_SECRET:-kalopaideia-$(openssl rand -hex 24)}"
...
# (Kalopaideia's commerce flow only needs the anon key for OTP / OAuth /
